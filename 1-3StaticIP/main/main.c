...
void app_main(void)
{
    ...
    //创建STA类型网卡
    esp_netif_t* pSta_Netif = esp_netif_create_default_wifi_sta();
    //停止DHCP客户端 DHCP: Dynamic Host Configuration Protocol
    //不让DHCP客户端自动获取IP，而是手动获取IP
    esp_netif_dhcpc_stop(pSta_Netif);

    //设置IP信息

    // esp_netif_ip_info_t ip_info;
    // ip_info.ip.addr = inet_addr("192.168.31.170");    //规定esp32的静态IP地址,inet_addr()函数将字符串转换为uint32_t类型的IP地址
    // ip_info.netmask.addr = inet_addr("255.255.255.0");//子网掩码
    // ip_info.gw.addr = inet_addr("192.168.31.1");      //网关

    esp_netif_ip_info_t ip_info =
    {
        .ip.addr = inet_addr("192.168.31.170"),    //规定esp32的静态IP地址,inet_addr()函数将字符串转换为uint32_t类型的IP地址
        .netmask.addr = inet_addr("255.255.255.0"),//子网掩码
        .gw.addr = inet_addr("192.168.31.1")       //网关
    };

    esp_netif_set_ip_info(pSta_Netif, &ip_info);
